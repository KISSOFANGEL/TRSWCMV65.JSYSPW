...
function CDateTimeValidator_validateDateFormat(_sDate, format){
	...
	if(_sDate==null){
		this.sErrorInfo += "对不起，您输入的信息为空！";
		return false;
	}
	if(format==null || format==""){
		this.sErrorInfo += "对不起，您指定了无效的日期格式！";
		return false;
	}
	...
	for(var i=0; i<formats.length; i++) {
		...
		if(regexp.test(value)){
			...
			if(year<1949){
				this.sErrorInfo += "[年] 必须大于 1949！";
				return false;
			}			
			if(month<0||month>12){
				this.sErrorInfo += "[月] 必须在 1-12 之间！";
				return false;
			}//end month
			if(day<0||day>31){
				this.sErrorInfo += "[日] 必须在 1-31 之间！";
				return false;
			}else{ 
				if(month==2){ 
					if(this.isLeapYear(year)&&day>29){
						this.sErrorInfo += "闰年的二月最多只有29天！";
						return false;
					}
					if(!this.isLeapYear(year)&&day>28){
						this.sErrorInfo += "闰年的二月有 1到28 天！";
						return false;
					}	
				}//end 2 month
				if((month==4||month==6||month==9||month==11)&&(day>30)){
					this.sErrorInfo += "当前月份最多只有30天！";
					return false;
				}//end 30days
			}//end day
			if(hour<0||hour>24){
				this.sErrorInfo += "[时] 必须在 1-24 之间！";
				return false;
			}//end hour
			if(minute<0||minute>60){
				this.sErrorInfo += "[分] 必须在 1-60 之间！";
				return false;
			}//end minute
			return true;
		}else if(i == formats.length-1){
			this.sErrorInfo += "您输入的日期 [" + _sDate + "] 不匹配合法的日期格式！ \n 一个正确的日期格式应当为：" + sDesc;
			return false;
		}//end formate
	}
	return true;
