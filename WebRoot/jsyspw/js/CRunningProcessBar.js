...
function RunningProcessBar_CheckClick(_bFirst){
	...
	if(m_nBgWidth == -1){
		m_nBgWidth = document.all.waiting.clientWidth;		
	}
...
//		}		
	...
	m_nCurrTime++;
	document.all.waitingTime.innerHTML = m_nCurrTime+"秒";
...
function RunningProcessBar_hideBodyChildren(_bDisplay){
	var arChildren = document.body.children;
...
function RunningProcessBar_draw(_sDesc){
	...
	var oWaitingDiv = document.all(sWaitingID);
	if(oWaitingDiv == null){
...
function RunningProcessBar_close(){
	...
	//2.Hide waiting
	var sWaitingID = "div_waiting";
	var oWaitingDiv = document.all("waiting");
	if(oWaitingDiv != null){
