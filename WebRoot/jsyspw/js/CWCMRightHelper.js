...
function CWCMRightHelper_draw(_nStartRightDefIndex, _nEndRightDefIndex, _elDiv,_sExtraIndexes){
	var sHTML = ""
			+"<TABLE width=\"98%\" border=\"0\" cellpadding=\"0\" cellspacing=\"1\" bgcolor=\"A6A6A6\">" 
			+"							<TR bgcolor=\"#BEE2FF\" class=\"list_th\">"			
			+"<TD bgcolor=\"#BEE2FF\">全选</TD>"
			+"<TD bgcolor=\"#BEE2FF\"><A href=\"#\" onClick=\"WCMRightHelper.sort();init();\">被设置者";
	...
	for(var i=0; i<this.arRight.length; i++){
		...
		sHTML	+= "<TR style=\"background-color:"+sBgColor+";font-size:9pt;\">\n"
				+"<TD align=center><input type=checkbox onclick=\"WCMRightHelper.selectAll("+i+","+_nEndRightDefIndex+", this.checked)\">"
				+(i+1)+"</TD>"
		...
		for(var j=0; j<arrayofIndexes.length; j++){
			...
			sHTML += "<TD  align=center title=\""+oRightDef["Desc"]+"\">"
					+ "<input type=checkbox onClick=\"CWCMRightHelper_onClickRightCheck(this, "+i+", "+nRightIndex+")\" value=\""+nRightIndex+"\" name=\""+_nEndRightDefIndex+"_"+oRight.getKey()+"\" "+(oRight.isHasRight(nRightIndex)?"checked":"")+">" + "</TD>";
		}
		sHTML += "<TD align=center><img alt='删除当前设置信息' src=\"../images/button_delete_small.gif\" onclick=\"WCMRightHelper.removeAt("+i+");CWCMRightHelper_notifyRemove(this);\" style=\"cursor:hand;\"></TD>";
