/** Title:			CWCMRightDefIndexValidator.js
 *  Description:
 *		Validate Site Name
 ...
 *	Examples:
 *			
 *	Use:（test/WCMRightDefIndexValidator_test.html）
 *	
...
<Script>
//register WCMRightDefIndex Validator
TRSValidator.addValidators("site_name",  "CWCMRightDefIndexValidator",  "../js/CWCMRightDefIndexValidator.js");    
</Script>
...
function CWCMRightDefIndexValidator(){
	this.validate				= CWCMRightDefIndexValidator_validate;
	this.isRightDefIndexExists	= CWCMRightDefIndexValidator_isRightDefIndexExists;
}

CWCMRightDefIndexValidator.inherits(CIntegerValidator);

function CWCMRightDefIndexValidator_validate(){
	//1. Super Validate
	...
	//3. Special Validate
	return !this.isRightDefIndexExists(this.oElement.value, this.oElement.RightDefId, this.oElement.ObjType);

}

function CWCMRightDefIndexValidator_isRightDefIndexExists(_sRightDefIndex, _nRightDefId, _nObjType){
	var sURL	 = "../auth/rightdef_get_by_name_index.jsp";
	var oTRSAction = new CTRSAction(sURL);
	oTRSAction.setParameter("ObjType", _nObjType);
	oTRSAction.setParameter("RightIndex", _sRightDefIndex);
	...
	if(bExists){
		this.sErrorInfo += "当前对象类型下的权限索引["+_sRightDefIndex+"]已存在！";
	}
	return bExists;
}