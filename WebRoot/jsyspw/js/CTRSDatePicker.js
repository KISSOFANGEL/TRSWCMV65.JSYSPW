function CTRSDatePicker_getValidDate(lYear,lMonth,lDay)
{
	var dt=new Date();
...
function CTRSDatePicker_initDatePicker(dp,dt)
{
	if(dp)
	{
		//Private Property
		...
		//Private Method
		dp.getDropDownTable=CTRSDatePicker_getDropDownTable;
		dp.getMonthName=CTRSDatePicker_getMonthName;
		dp.hideDropDown=CTRSDatePicker_hideDropDown;
		dp.initDropDown=CTRSDatePicker_initDropDown;
		dp.onDateChange=CTRSDatePicker_onDateChange;
		dp.refreshPostText=CTRSDatePicker_refreshPostText;
		dp.showDropDown=CTRSDatePicker_showDropDown;
		...
		//Public Method
		dp.setFocus=CTRSDatePicker_setFocus;
		dp.format=CTRSDatePicker_format;
		dp.getDateContent=CTRSDatePicker_getDateContent;
		dp.getDay=CTRSDatePicker_getDay;
		dp.getEnabled=CTRSDatePicker_getEnabled;
		dp.getMonth=CTRSDatePicker_getMonth;
		dp.getYear=CTRSDatePicker_getYear;
		dp.refreshView=CTRSDatePicker_refreshView;
		dp.setAccessKey=CTRSDatePicker_setAccessKey;
		dp.setCurDate=CTRSDatePicker_setCurDate;
		dp.setDateDes=CTRSDatePicker_setDateDes;
		dp.setEnabled=CTRSDatePicker_setEnabled;
		dp.setFormat=CTRSDatePicker_setFormat;
		dp.setMaxDate=CTRSDatePicker_setMaxDate;
		dp.setMinDate=CTRSDatePicker_setMinDate;
		dp.setTabIndex=CTRSDatePicker_setTabIndex;
		dp.setWeekName=CTRSDatePicker_setWeekName;
...
function CTRSDatePicker_createDropDown()
{
	...
	document.body.insertAdjacentHTML("BeforeEnd",
					...
					"onclick=\"CTRSDatePicker_ddt_click();\" "+
					"ondblclick=\"CTRSDatePicker_ddt_dblclick();\">"+
					"<TR class=DPTitle>"+
					"<TD><span class=DPBtn onclick=\"CTRSDatePicker_monthChange(-1);\" title=\"上月\">《</span></TD>"+
					"<TD align=center colspan=5></TD>"+
					"<TD><span class=DPBtn onclick=\"CTRSDatePicker_monthChange(1);\" title=\"下月\">》</span></TD>"+
					"</TR>"+
					"<TR>"+
...
function CTRSDatePicker_getYear()
{
	var dp=this;
	return dp.curDate.getFullYear();
}

function CTRSDatePicker_getMonth()
{
	var dp=this;
	return dp.curDate.getMonth()+1;
}

function CTRSDatePicker_getDay()
{
	var dp=this;
	return dp.curDate.getDate();
}

function CTRSDatePicker_format(sFormat)
{
	var dp=this;
	return dp.curDate.format(sFormat);
}

function CTRSDatePicker_setAccessKey(sKey)
{
	var dp=this;
...
function CTRSDatePicker_getEnabled()
{
	var dp=this;
	var val=false;
...
function CTRSDatePicker_setEnabled(val)
{
	var dp=this;
	var hr=false;
...
function CTRSDatePicker_setFocus()
{
	var dp=this;
	var src=dp.children[0];
...
function CTRSDatePicker_getDateContent()
{
	var dp=this;
	var con="";
	var sYearDes="";
...
function CTRSDatePicker_setFormat(sFormat)
{
	this.formatString=sFormat;
	this.refreshPostText();
}

function CTRSDatePicker_refreshPostText()
{
	var dp=this;
...
function CTRSDatePicker_initDropDown()
{
	var dp=this;
...
function CTRSDatePicker_getMonthName(lMonth)
{
	var mnArr=new Array("一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月");
	return mnArr[lMonth];
}

function CTRSDatePicker_setWeekName()
{
	var dp=this;
	var ddt=dp.getDropDownTable();
	if(ddt)
...
function CTRSDatePicker_showDropDown()
{
	var dp=this;
	var ddt=dp.getDropDownTable();
	...
	if(ddt)
	{
		...
		else
		{
			...
			document.onclick=CTRSDatePicker_sub_docClick;
			document.onkeydown=CTRSDatePicker_sub_dockeydown;
...
function CTRSDatePicker_hideDropDown()
{
	var ddt=getDropDownTable();
	if(ddt)
	{
...
function CTRSDatePicker_getDropDownTable()
{
	var dp=this;
	dp.dropDownTable=CTRSDatePicker_createDropDown();
...
function CTRSDatePicker_onDateChange()
{
	var dp=this;
...
function CTRSDatePicker_refreshView()
{
	var dp=this;
	var hr=false;
...
function CTRSDatePicker_setTabIndex(lTabIndex)
{
	var dp=this;
	var hr=false;
...
function CTRSDatePicker_setDateDes(sYearDes,sMonthDes,sDayDes)
{
	if(sYearDes==null)
		sYearDes="-";
	if(sMonthDes==null)
...
function CTRSDatePicker_setMaxDate(lYear,lMonth,lDay)
{
	var dp=this;
	var hr=false;
...
function CTRSDatePicker_setMinDate(lYear,lMonth,lDay)
{
	var dp=this;
	var hr=false;
...
function CTRSDatePicker_setCurDate(lYear,lMonth,lDay)
{
	var dp=this;
	var hr=false;
...
function CTRSDatePicker_DropClick()
{
...
function CTRSDatePicker_focus(srcType)
{
	var src=event.srcElement;
	if(src && src.tagName=="INPUT")
	{
...
function CTRSDatePicker_blur(srcType)
{
	var src=event.srcElement;
	var dp=getParentFromSrc(src,"SPAN")
...
function CTRSDatePicker_keyDown(srcType)
{
	var src=event.srcElement;
	var dp=getParentFromSrc(src,"SPAN")
	var bRefresh=true;
...
function CTRSDatePicker_monthChange(lStep)
{
	var src=event.srcElement;
	if(src)
	{
...
function CTRSDatePicker_ddt_click()
{
	var src=event.srcElement;
		...
		if(ddt && ddt.dp)
		{
			...
			if(ddt.dp.setCurDate(src.year,parseInt(src.month,10)+1,parseInt(src.innerText,10)))
			{
				if(src.month!=lOldMonth)
					...
					CTRSDatePicker_onCell(src);
			}
		}
	}
}

function CTRSDatePicker_onCell(src)
{
...
function CTRSDatePicker_ddt_dblclick()
{
	var src=event.srcElement;
	if(src && src.tagName=="TD")
	{
...
function CTRSDatePicker_sub_docClick()
{
	var src=event.srcElement;
	var ddt=getParentFromSrc(src,"TABLE");
	if(!ddt || ddt.id!="dpDropDownTable")
	{
		CTRSDatePicker_hideDropDown();
	}
	event.cancelBubble=true;
	event.returnValue=false;
...
function CTRSDatePicker_sub_dockeydown()
{
	CTRSDatePicker_hideDropDown();
	return true;
}
...
function CTRSDatePicker_createDTPicker(_sDateElementName, dateStr) {
	var aDate = new Array();
	aDate = dateStr.split("-");
	this.createDatePicker(_sDateElementName, aDate[0], aDate[1], aDate[2]);
}

function CTRSDatePicker_createDatePicker(_sDateElementName, lYear,lMonth,lDay)
{
	var dpID="CTRSDatePicker_"+(m_iDatePickerCount++);
	var dt=CTRSDatePicker_getValidDate(lYear,lMonth,lDay);
	if(dt==null)
		dt=new Date();

	document.write("<DIV style=\"display:inline\" NOWRAP>");
	document.write("<span class=DPFrame id="+dpID+">");
	document.write("<input class=DPYear type=text value="+dt.getFullYear()+" size=4 maxlength=4 onfocus=\"return CTRSDatePicker_focus('year');\" onblur=\"return CTRSDatePicker_blur('year');\" onkeypress=\"return KeyFilter('number');\" onkeydown=\"return CTRSDatePicker_keyDown('year');\">");
	document.write("<font class=DPYearDes>年</font>");
	document.write("<input class=DPMonth type=text value="+(dt.getMonth()+1)+" size=2 maxlength=2 onfocus=\"return CTRSDatePicker_focus('month');\" onblur=\"return CTRSDatePicker_blur('month');\" onkeypress=\"return KeyFilter('number');\" onkeydown=\"return CTRSDatePicker_keyDown('month');\">");
	document.write("<font class=DPMonthDes>月</font>");
	document.write("<input class=DPDay type=text value="+dt.getDate()+" size=2 maxlength=2 onfocus=\"return CTRSDatePicker_focus('day');\" onblur=\"return CTRSDatePicker_blur('day');\" onkeypress=\"return KeyFilter('number');\" onkeydown=\"return CTRSDatePicker_keyDown('day');\">");
	document.write("<font class=DPDayDes>日</font>");
	document.write("<span class=DPSep></span>");
	document.write("<a class=DPDropBtn href=\"\" onclick=\"CTRSDatePicker_DropClick();return false;\" title=\"选择日期\">▼</a>");
	document.write("</DIV>");
	if(typeof(_sDateElementName)=="string" && _sDateElementName.length>0)
	{
		document.write("<input type=hidden value='"+dt.format("yyyy-mm-dd")+"'  id=Edit_"+_sDateElementName+" name="+_sDateElementName+">");
	}
	document.write("</span>");

	var dp=document.all(dpID);
	CTRSDatePicker_initDatePicker(dp,dt);
	return dp;
}
...
function CTRSDatePicker(){
}

CTRSDatePicker.prototype.createDatePicker	= CTRSDatePicker_createDatePicker;
CTRSDatePicker.prototype.createDTPicker		= CTRSDatePicker_createDTPicker;

//Define Default Instance
var TRSDatePicker = new CTRSDatePicker();
